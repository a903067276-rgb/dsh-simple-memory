window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function MemorySettingsPage(props) {
      ...
      // 可调项（2026-09-26 加）：自动提交开关 + 定期做梦的间隔与条数
      const [autoCommit, setAutoCommit] = react.useState(true);
      const [dreamDays, setDreamDays] = react.useState(7);
      const [dreamBatch, setDreamBatch] = react.useState(2);
      ...
      async function loadConfig() {
        try {
          ...
          if (r && r.ok) {
            setDirDraft(r.globalMemoryDir || "");
            if (typeof r.autoCommit === "boolean") setAutoCommit(r.autoCommit);
            if (typeof r.dreamEveryDays === "number") setDreamDays(r.dreamEveryDays);
            if (typeof r.dreamBatch === "number") setDreamBatch(r.dreamBatch);
          }
        } catch (e) { /* 静默：读不到就保持空 */ }
      }
      ...
      async function saveConfig() {
        ...
        try {
          const r = await api("/config", null, "POST", {
            globalMemoryDir: dir,
            autoCommit,
            dreamEveryDays: dreamDays,
            dreamBatch,
          });
      ...
      return react.createElement("div", { className: "smem-card" },
        ...
        react.createElement("div", { className: "row", style: { marginTop: 6, fontSize: 12 } },
          react.createElement("label", { style: { display: "inline-flex", alignItems: "center", gap: 6, cursor: "pointer" } },
            ...
            "写入后自动 git 提交（推荐；关掉就回到『靠模型记得提交』）"
          )
        ),
        react.createElement("div", { className: "row", style: { marginTop: 4, fontSize: 12 } },
          react.createElement("label", { style: { marginRight: 6 } }, "定期做梦：每"),
          react.createElement("input", {
            type: "number", min: 1, max: 365, value: String(dreamDays),
            onChange: (e) => setDreamDays(Number(e.target.value)),
            style: { width: 64, padding: "3px 6px", borderRadius: 6, border: "1px solid var(--dsw-alias-border-l1,#e5e5e5)", background: "transparent", color: "var(--dsw-alias-label-primary,#333)", fontFamily: "inherit" },
          }),
          "天一次，每次做",
          react.createElement("input", {
            type: "number", min: 0, max: 5, value: String(dreamBatch),
            onChange: (e) => setDreamBatch(Number(e.target.value)),
            style: { width: 48, marginLeft: 6, padding: "3px 6px", borderRadius: 6, border: "1px solid var(--dsw-alias-border-l1,#e5e5e5)", background: "transparent", color: "var(--dsw-alias-label-primary,#333)", fontFamily: "inherit" },
          }),
          "个梦（到期会在会话开头提醒我；产出进做梦池等你确认）"
        ),
    ...
    function apply(ctx) {
      ...
      const ROW_CONFIG_KEY = "dsh-simple-memory#simple-memory";
      slots.inject("plugins.row.config", () => slots.register(
        { name: "plugins.row.config", key: ROW_CONFIG_KEY, order: 30, label: "记忆" },
      ...
      slots.inject("plugins.bundle.config", () => slots.register(
        { name: "plugins.bundle.config", key: "dsh-simple-memory", order: 30, label: "记忆" },
        () => react.createElement(MemorySettingsPage, { inCard: true }),
      ));
