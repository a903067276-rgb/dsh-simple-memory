window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const INSTRUCTION =
      "现在执行记忆流程（按全局 AGENTS.md「记忆规范」）：① 回顾本轮对话，列出值得记的内容（决策/踩坑/新约定/偏好）② 逐条提议并标明每条记到哪：项目（仅本项目有用）或 全局（跨项目复用），各附一句理由，等用户确认 ③ 确认后调用 memory-write 工具写入（scope 与提议一致）④ 贴出产出物 ⑤ 顺手 git commit（mem: 记 xxx）";
