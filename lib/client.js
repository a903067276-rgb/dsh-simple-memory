window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const PROMPT_DREAM =
      "现在做一次「做梦」（随机组合记忆找洞察）：① 从记忆库随机抽 3~5 条（尽量跨项目、跨分类，优先没组合过的；可用 memory_search/读文件） ② 尝试建立非常规连接，重点找四类：共性模式（不同项目的坑其实是同一根因 → 可提炼通用原则）/ 矛盾过时（两条记忆冲突，或某条已被新实践推翻）/ 迁移机会（A 项目的解法正好能用到现在做的事）/ 空白（反复遇到却没记过的模式） ③ **最多产出 2~3 条**洞察，宁缺毋滥；每条给出：入场的记忆清单 + 梦到的连接 + 建议去向（全局 or 哪个项目）+ 一句价值判断 ④ 把产出追加写入记忆库根目录 dreams.md，每条一行、严格按格式：`- 日期 [待确认] 入场：记忆A、记忆B、记忆C → 连接：xxx（类型：共性/矛盾/迁移/空白）→ 建议去向：全局 or 项目X → 价值：一句话`；**不要直接写进 common/ 或 projects/** ⑤ 告诉我池里新增了几条、分别是什么，等我看完决定升格还是弃用";
    ...
    if (typeof document !== "undefined" && !document.getElementById("dsh-simple-memory-style")) {
      ...
      tag.textContent = [
        ...
        // ── 四动作菜单 + 记忆浏览浮层（2026-09-12；浮层只用官方 token：bg-overlay / border-l1|l2 / label-*）──
        ".smem-wrap{position:relative;display:inline-flex;}",
        ".smem-menu{position:absolute;bottom:calc(100% + 6px);left:0;z-index:60;min-width:196px;padding:4px;border:1px solid var(--dsw-alias-border-l2,#d0d0d0);border-radius:10px;background:var(--dsw-alias-bg-overlay,#fff);box-shadow:0 6px 20px rgba(0,0,0,.14);}",
        ".smem-menu-item{display:flex;flex-direction:column;gap:1px;width:100%;text-align:left;border:none;background:transparent;color:var(--dsw-alias-label-primary,#333);padding:7px 9px;border-radius:7px;cursor:pointer;font:inherit;font-size:12px;}",
        ".smem-menu-item:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(0,0,0,.06));}",
        ".smem-menu-item .t{font-weight:600;font-size:12px;}",
        ".smem-menu-item .d{color:var(--dsw-alias-label-tertiary,#999);font-size:11px;}",
        ".smem-float{position:absolute;bottom:calc(100% + 6px);left:0;z-index:60;width:340px;max-height:54vh;overflow:auto;padding:10px;border:1px solid var(--dsw-alias-border-l2,#d0d0d0);border-radius:10px;background:var(--dsw-alias-bg-overlay,#fff);box-shadow:0 6px 20px rgba(0,0,0,.14);color:var(--dsw-alias-label-primary,#333);font-size:12px;}",
        ".smem-float .head{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:6px;}",
        ".smem-float .title{font-weight:600;font-size:12px;}",
        ".smem-float .close{border:none;background:transparent;color:var(--dsw-alias-label-tertiary,#999);cursor:pointer;font:inherit;font-size:14px;line-height:1;padding:2px 5px;border-radius:6px;}",
        ".smem-float .close:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(0,0,0,.06));}",
        ".smem-float .pools{color:var(--dsw-alias-label-tertiary,#999);font-size:11px;margin-bottom:6px;}",
        ".smem-float .searchbox{width:100%;box-sizing:border-box;padding:5px 8px;border-radius:7px;border:1px solid var(--dsw-alias-border-l1,#e5e5e5);background:transparent;color:inherit;font:inherit;font-size:12px;margin-bottom:6px;}",
        ".smem-float .sec{margin:7px 0 2px;color:var(--dsw-alias-label-tertiary,#999);font-size:11px;}",
        ".smem-float .f{display:flex;align-items:center;gap:5px;padding:3px 4px;border-radius:6px;cursor:pointer;color:var(--dsw-alias-label-secondary,#666);}",
        ".smem-float .f:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(0,0,0,.06));color:var(--dsw-alias-label-primary,#333);}",
        ".smem-float .empty{color:var(--dsw-alias-label-tertiary,#999);padding:4px 0;}",
        ".smem-float .body{margin-top:6px;padding-top:6px;border-top:1px solid var(--dsw-alias-border-l1,#e5e5e5);white-space:pre-wrap;word-break:break-word;color:var(--dsw-alias-label-secondary,#666);max-height:26vh;overflow:auto;font-size:12px;}",
        ...
        (props) => {
          ...
          react.useEffect(() => {
            ...
            function onDocDown(e) {
              if (wrapRef.current !== null && !wrapRef.current.contains(e.target)) setMenuOpen(false);
            }
