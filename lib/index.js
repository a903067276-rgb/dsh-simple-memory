...
export function apply(ctx, rawConfig) {
  ...
  /** 做梦配置的取值（卡片读取与自检共用）。 */
  const dreamEveryDaysValue = () => {
    const n = Number(refValue(cfgIn.dreamEveryDays))
    return Number.isFinite(n) && n > 0 ? n : 7
  }
  const dreamBatchValue = () => {
    const n = Number(refValue(cfgIn.dreamBatch))
   ...
   */
  /** 通用配置保存（2026-09-26）：一次写多个键；老宿主无 settings.update 时只有目录能落地。 */
  async function saveMemoryConfig(patch) {
    ...
    if (typeof patch.globalMemoryDir === 'string') {
      writeFileSync(PATCH_PATH, updatePatchGlobalMemoryDir(patch.globalMemoryDir))
      return
    }
    throw new Error('当前宿主不支持写入插件配置（需要 0.1.7 的 settings.update）')
  }
        ...
        try {
          ...
          if (pathname === '/api/dsh-simple-memory/config') {
            const currentConfig = () => ({
              ...
              dreamEveryDays: dreamEveryDaysValue(),
              dreamBatch: dreamBatchValue(),
            ...
            if (req.method === 'POST' || req.method === 'PUT') {
              ...
              let input
              try { input = JSON.parse(bodyText || '{}') } catch (e) { input = null }
              if (input === null || typeof input !== 'object') {
                writeJson(res, 400, { ok: false, error: 'body 必须是 JSON 对象' })
                return
              }
              const patch = {}
              if (Object.hasOwn(input, 'globalMemoryDir')) {
                const target = String(input.globalMemoryDir || '').trim()
                if (!target.startsWith('/') && !/^[A-Za-z]:[\\/]/.test(target)) {
                  writeJson(res, 400, { ok: false, error: '记忆根目录必须是绝对路径（如 /Users/xxx/Documents/DSH/memory）' })
                  return
                }
                patch.globalMemoryDir = target
              }
              if (Object.hasOwn(input, 'autoCommit')) patch.autoCommit = input.autoCommit !== false
              if (Object.hasOwn(input, 'dreamEveryDays')) {
                const n = Number(input.dreamEveryDays)
                patch.dreamEveryDays = Number.isFinite(n) && n >= 1 ? Math.min(365, Math.round(n)) : 7
              }
              if (Object.hasOwn(input, 'dreamBatch')) {
                const n = Number(input.dreamBatch)
                patch.dreamBatch = Number.isFinite(n) && n >= 0 ? Math.min(5, Math.round(n)) : 2
              }
              if (Object.keys(patch).length === 0) {
                writeJson(res, 400, { ok: false, error: '没有可保存的字段' })
                return
              }
              try {
                await saveMemoryConfig(patch)
                // 回"写完后真读到的值"（不是请求体回显）
                writeJson(res, 200, Object.assign(currentConfig(), { message: '已生效（无需重启）' }))
              } catch (e) {
