...
export function apply(ctx, config) {
  ...
  /** 做梦池条目数（同步版，2026-09-12 新增；池子非空 → 索引尾部提示待确认）。 */
  function dreamsCountSync() {
    try {
      const text = readFileSync(join(GLOBAL_MEMORY_DIR, 'dreams.md'), 'utf8')
      return text.split('\n').filter((l) => /^\s*-\s+\d{4}-\d{2}-\d{2}/.test(l)).length
    } catch (e) { return 0 }
  }
  ...
  async function dreamsCountAsync() {
    try {
      const text = await readFileText(join(GLOBAL_MEMORY_DIR, 'dreams.md'))
      if (text === undefined) return 0
      const lines = text.split('\n').filter((l) => /^- \d{4}-\d{2}-\d{2}/.test(l))
      return lines.length
    } catch (e) { return 0 }
      ...
      handler: async (req, res) => {
        try {
          ...
          if (pathname === '/api/dsh-simple-memory/init') {
            ...
            const dreamsPath = join(GLOBAL_MEMORY_DIR, 'dreams.md')
            if (!existsSync(dreamsPath)) {
              await fs.writeText(await fs.resolve(dreamsPath), [
                ...
                '## 治理',
                '',
                '- 状态取值：[待确认] / [已采纳] / [已弃]；已采纳 → 升格写入后从此处删除该条',
                '- 已弃的留一行痕迹（防重复梦同一组合）；池子 >15 条 → 提醒整理',
                '',
              ].join('\n'))
