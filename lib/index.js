...
const NS = settingsNamespace('simple-memory')
...
export function apply(ctx, config) {
  ...
  async function buildIndex(cwd) {
    ...
    return '【记忆索引】相关时按需读全文（单文件 ≤2KB），先活跃后冷区（references/、archive/ 命中搜索才读）：\n'
      + lines.join('\n')
      + '\n遇到值得沉淀的内容（决策/踩坑/新约定/偏好）时自然主动提议记录（不要每轮机械提醒，由你判断时机）：先列候选清单（标明项目或全局）等用户确认，确认后用 memory-write 写入（写后贴产出物、git commit）。'
  }
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    name: 'memory-write',
    description: '按记忆规范强制格式写入一条记忆：文件名必须为 分类-主题.md（分类自造，内置踩坑/流程/决策/偏好/背景），内容 ≤2KB，首行 ## 日期 分类-主题。scope=project 写入 <项目>/memory/，scope=global 写入全局 common/。写入前必须已获用户确认。沙箱拒绝后可用 sandbox_permissions+justification 一次性提权重试（global 写工作区外必弹审批）。调用时请带上 path 参数（目标路径，权限申报用，可让 perm-guard 自动放行免弹窗）。',
    parameters: {
