...
export function apply(ctx, rawConfig) {
  ...
  /** 记忆根目录执行 git add -A && git commit（任何失败都吞掉，绝不影响写入结果）。 */
  async function commitMemory(action, subject) {
    if (!autoCommitOn() || shell === undefined) return
    const dir = memDir()
    ...
    try {
      ...
      // 没有改动可提交 / git 不可用 / 钩子失败——都不该影响记忆写入本身
    }
  }
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      ...
      try {
        ...
        await commitMemory('写入', fileName)
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      ...
      try {
        ...
        await commitMemory('进度', targetProject)
