...
- **设置页可改（v0.7.0）**：记忆根目录之外，**自动提交开关**、**定期做梦的间隔与条数**也在设置卡片里改，保存走 profile 条目 config（volatile → 即时生效、不用重启）。
...
- **定期自动做梦（2026-09-26 新增）**：`dreamEveryDays`（默认 7 天）到期时，插件在**当次会话的首步注入**里点名：本轮做 `dreamBatch`（默认 2）个梦。产出照旧写进 `dreams.md`、条目带 `[自动][待确认]` 便于和手动梦区分。**不在后台偷偷发 LLM 请求**（也就不在你没看着的时候花钱），只在有会话时提醒；上次触发时间记在 `<记忆根>/.dream-state.json`。
